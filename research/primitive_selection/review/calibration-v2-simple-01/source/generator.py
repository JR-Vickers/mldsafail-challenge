...
def generate_msis(profile: str, seed: int, eta: int) -> GeneratedMSIS:
    ...
    a_rng = _rng("msis", profile, seed, "matrix", eta)
    z_rng = _rng("msis", profile, seed, "relation", eta)
    ...
    draft = MSISInstance(profile, n, q, rows, columns, eta, A, norm_squared, norm_infinity)
    public = MSISInstance(profile, n, q, rows, columns, eta, A,
