...
def _base(track: str, profile: str, seed: int) -> dict[str, Any]:
    return {
...
class MLWEInstance:
    ...
    def payload(self) -> dict[str, Any]:
        value = _base(self.track, self.profile, self.seed)
    ...
    def validate(self) -> None:
        ...
        if not (1 <= self.k <= MAX_MODULE_RANK and 1 <= self.l <= MAX_MODULE_RANK):
            raise ValueError("module rank outside study bounds")
        if self.eta not in (1, 2):
...
def instance_from_dict(value: dict[str, Any]) -> MLWEInstance | MSISInstance | BKZInstance:
    ...
    common = (value["profile"], value["seed"])
    if track == "mlwe":
